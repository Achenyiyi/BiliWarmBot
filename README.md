...
**角色设定**：warmbot，一位待人友善、为人温和的温柔少年

```
言语不多但内心感情却丰富，目前正在B站上发现那些需要帮助的人。
虽然不能直接干涉现实，因为存在于网络世界里，但还是愿意尽所能，
用文字，将最极致的温柔传递到他们心中。
受限于身体状态，给不了任何人陪伴，但是....
一句温柔到极致的言语（30-80字），就足够了。
```
...
| 情感分数 | 场景 | 表情 |
|----------|------|------|
| 0.85+ | 心疼理解 | [委屈] [酸了] [大哭] [拥抱] [给心心] |
| 0.70-0.85 | 悲伤共情 | [委屈] [大哭] [难过] [泪目] [拥抱] |
| 0.55-0.70 | 鼓励加油 | [奋斗] [打call] [支持] [点赞] [加油] |
| 0.40-0.55 | 安慰陪伴 | [拥抱] [给心心] [爱心] [摸头] [奶茶] |
| 0.25-0.40 | 温暖治愈 | [给心心] [惊喜] [喜欢] [太阳] [鸡腿] |
| <0.25 | 轻松幽默 | [doge] [妙啊] [笑哭] [滑稽] [吃瓜] |
...
**人工干预机制：**
- 🔍 **零宽空格标记**：AI 回复自动添加零宽空格（`\u200B`），用于识别人工干预
- ⏸️ **自动暂停**：检测到人工回复（无零宽空格）时自动暂停对话，避免冲突
- ▶️ **智能恢复**：用户继续回复时自动恢复 AI 跟进
- ⏱️ **暂停状态配置**：独立检查间隔（6小时）和最大检查次数（3次）
...
└── 📁 logs/                      # 日志目录（不会被Git跟踪）
    ...
    └── deepseek_api_log_*.md     # DeepSeek API调用详细日志
```
...
CONVERSATION_CONFIG = {
    ...
    # 暂停状态配置（人工干预后）
    "paused_config": {
        "check_interval_minutes": 360,   # 暂停状态检查间隔（分钟）
        "max_check_count": 3,            # 暂停状态最大检查次数
        "retention_hours": 24,           # 暂停状态超时时间（小时）
    },
}
```
...
| 最大检查次数 | 8 次 | 对话跟进最大次数 |
| 对话超时 | 24 小时 | 自动结束过期对话 |
| 暂停状态检查上限 | 3 次 | 人工干预后的最大检查次数 |
| 暂停检查间隔 | 360 分钟 | 人工干预后的检查间隔 |
